...
function EmotionCard({ index, title, description, icon, iconX, iconY }) {
    return (
        <span className="rounded-3xl bg-white border-greenDark w-full h-40 md:h-52 flex flex-col p-7 pr-12 relative font-montserrat font-normal">
            <div className={"flex flex-col justify-between h-full w-3/4 p-3 text-left"}>
            <p className={"font-montserrat font-normal"}>
                / 0{index}
            </p>
            </div>
            <div className="flex flex-col justify-between h-full z-10">
                <p className="font-montserrat font-normal -mt-1">
                    {title}
                </p>
            ...
            {icon && (
                ...
                <img
                    src={icon}
                    alt="icon"
                    className={"object-contain absolute right-4 top-1/2 transform -translate-y-1/ max-h-[80px] max-w-[80%] md:max-h-[150px] md:max-w-[150px]"}
